import sys
sys.path.insert(1, '../LocalBoulders')
from dataStructure import Photo, Topo, AreaMap, SubAreaMap
from GB_Book import book


# Photo(name='Austin Powers',
#       fileName='AustinPowers.jpg',
#       parent=book.formations['Mini Me'],
#       route=book.routes['austinPowers'],
#       credit='Andrew Child',
#       description='Carson cranking across the face on Austin Powers.'),
# Photo(name='Riptide',
#       fileName='Riptide.jpg',
#       parent=book.formations['Undertow'],
#       size = 'h',
#       loc = 'e',
#       route=book.routes['Riptide'],
#       credit='Andrew Child',
#       description='Rob on Riptide.')
# Photo(name='Fight Club',
#       fileName='FightClub2.jpg',
#       parent=book.subareas['Fight Club'],
#       size='h',
#       loc='e',
#       route=book.routes['Fight Club'],
#       credit='Andrew Child',
#       description='Michael near the top of Fight Club.')
# Photo(name='Octernal',
#       fileName='Octurnal.jpg',
#       parent=book.subareas['Undertow'],
#       size='s',
#       loc='e',
#       route=book.routes['Octernal'],
#       credit='Andrew Child',
#       description='Carson landing the big throw on Octernal. Classic!')
# Photo(name='Smackdown',
#       fileName='Smackdown.jpg',
#       parent=book.subareas['François'],
#       route=book.routes['Smackdown'],
#       size='f',
#       loc='e',
#       credit='Carson Dension',
#       description='Andrew posting up at the start of Smackdown.')
# Photo(name='Ground up Blowie',
#       fileName='blowie.jpg',
#       parent=book.formations['Toilet Bowl'],
#       size='p',
#       loc='e',
#       route=book.routes['Ground up Blowie'],
#       credit='Michael Gardener Brown',
#       description='Andrew strugling to find a finger lock on Ground up Blowie.')
# Photo(name='Butterfly',
#       fileName='butterfly.jpg',
#       parent=book.subareas['The Garden Cliff'],
#       size='f',
#       loc='e',
#       route=book.routes['Butterfly Effect'],
#       credit='Andrew Child',
#       description='Griffin on Butterfly Effect.')
# Photo(name='2blows',
#       fileName='2blows.jpg',
#       parent=book.subareas['Azain'],
#       size='h',
#       loc='e',
#       route=book.routes['Two Blows One Stroke'],
#       credit='Andrew Child',
#       description='Griffin on Two Blows One Stroke.')
      
Topo(name='Bitchin Corners',
     parent=book.formations['Bitchin Corners'],
     fileName='bitchin.svg',
     size='h',
      loc='e',
     border='rect290',
     routes={
         'path1002': book.routes['Bitchin Corners'],
         'path1029': book.routes['Bitchin Corners'],
         'path1014': book.variations['Bitchin Corners Sit'],
         'path1031': book.variations['Bitchin Corners Sit'],
     })
Topo(name='Baldo',
     parent=book.formations['Baldo'],
     fileName='baldo.svg',
     size='h',
     routes={
         'path600': book.routes['Frontside Baldo'],
         'path598': book.routes['Frontside Baldo'],
     })
Topo(name='Berned',
     parent=book.formations['Big'],
     fileName='berned.svg',
     size='h',
     routes={
         'path744': book.routes['All Berned Up'],
         'path742': book.routes['All Berned Up'],
         'path740': book.routes['All Berned Down'],
         'path738': book.routes['All Berned Down'],
     })
Topo(name='Classique',
     parent=book.formations['Classique'],
     fileName='classique.svg',
     size='h',
     routes={
         'path445': book.routes['Classique'],
         'path391': book.routes['Classique'],
         'path293': book.routes['Classique'],
     })
Topo(name='Crazy Cool',
     parent=book.formations['Crazy Cool'],
     fileName='crazyCool.svg',
     size='h',
     routes={
         'path586': book.routes['Crazy Cool Arête'],
         'path584': book.routes['Crazy Cool Arête'],
         'path582': book.routes['Crazy Cool Arête'],
     })
Topo(name='Overhand',
     parent=book.formations['Overhand'],
     fileName='overhand2.svg',
     size='h',
     routes={
         'path1177': book.routes['Overhand'],
         'path1175': book.routes['Overhand'],
     })
Topo(name='Hueco Wabo',
     parent=book.formations['Hueco Wabo'],
     fileName='hueco.svg',
     size='h',
     routes={
         'path1918': book.routes['Hueco Wabo'],
         'path1920': book.routes['Hueco Wabo'],
     })
Topo(name='Azain Spire',
     parent=book.formations['Azain Spire'],
     fileName='azainSpire.svg',
     size='h',
     routes={
         'path724': book.routes['Snakes and Martyrs'],
         'path726': book.routes['Snakes and Martyrs'],
     })
Topo(name='Three Star Ledge',
     parent=book.formations['Three Star Ledge'],
     fileName='threeStar2.svg',
     size='h',
     routes={
         'path2023': book.routes['Three Star Ledge'],
         'path2021': book.variations['Three Star Ledge Variation'],
         'path2019': book.variations['Three Star Ledge Variation'],
         'path2017': book.variations['Three Star Ledge Variation'],
     })
Topo(name='The Good',
     parent=book.routes['The Good'],
     fileName='good.svg',
     size='f',
     routes={
         'path1453': book.routes['The Good'],
         'path1459': book.routes['The Good'],
         'path1451': book.routes['Another'],
         'path1457': book.routes['Another'],
         'path1445': book.routes['Next to the Good'],
         'path1447': book.routes['Next to the Good'],
         'path1449': book.routes['Next to the Good'],
         'path1455': book.routes['The Good Slab'],
         'path1461': book.routes['The Good Slab'],
     })
Topo(name='Fight Club Right Side',
     parent=book.formations['Fight Club'],
     fileName='fightClub.svg',
     size='h',
     border='rect789',
     routes={
         'path735': book.routes['Fight Club 2'],
         'path733': book.routes['Fight Club 2'],
         'path731': book.routes['The Ear'],
         'path729': book.routes['The Ear'],
         'path727': book.routes['Fight Club'],
         'path725': book.routes['Fight Club'],
     })
Topo(name='Fight Club Left Side',
     parent=book.routes['Fight Club 2'],
     fileName='fightClub2.svg',
     size='h',
     routes={
         'path1297': book.routes['Fight Club 2'],
         'path1303': book.routes['Fight Club 2'],
         'path1299': book.routes['Fight Club'],
         'path1301': book.routes['Fight Club'],
         'path1293': book.routes['Brewmaster'],
         'path1305': book.routes['Brewmaster'],
     })
Topo(name='E\'s Dirty B',
     parent=book.formations['E\'s Dirty B'],
     fileName='eDirty.svg',
     size='h',
     routes={
         'path446': book.routes['Green Hell'],
         'path290': book.routes['Green Hell'],
         'path1003': book.routes['E\'s Dirty B'],
         'path1005': book.routes['E\'s Dirty B'],
         'path444': book.routes['Unknown on E\'s Dirty B'],
         'path388': book.routes['Unknown on E\'s Dirty B'],
     })
Topo(name='Car Alarm Downhill Side',
     parent=book.routes['White Rhino'],
     fileName='carAlarm.svg',
     size='f',
     routes={
         'path674': book.routes['Car Alarm Traverse'],
         'path676': book.routes['Car Alarm Traverse'],
         'path670': book.routes['White Rhino'],
         'path678': book.routes['White Rhino'],
         'path668': book.routes['2 Ton Chevey'],
         'path680': book.routes['2 Ton Chevey'],
         'path666': book.routes['Pup Truck'],
         'path682': book.routes['Pup Truck'],
     })
Topo(name='Car Alarm Uphill Side',
     parent=book.routes['Comp Route'],
     fileName='carAlarm2.svg',
     size='h',
     routes={
         'path824': book.routes['Comp Route'],
         'path863': book.routes['Comp Route'],
         'path851': book.routes['Panic Button'],
         'path861': book.routes['Panic Button'],
         'path857': book.variations['Panic Button Variation'],
         'path859': book.variations['Panic Button Variation'],
     })
Topo(name='Boys in the Woods',
     parent=book.formations['Boys In the Woods'],
     fileName='BITW.svg',
     size='f',
     routes={
         'path1322': book.variations['Tabor Tots'],
         'path1320': book.variations['Tabor Tots'],
         'path300': book.routes['Boys in the Woods'],
         'path458': book.routes['Boys in the Woods'],
         'path1485': book.routes['Cuba Gooding'],
         'path460': book.routes['Cuba Gooding'],
         'path400': book.routes['Ice Cubes Shiny Jerry Curl'],
         'path462': book.routes['Ice Cubes Shiny Jerry Curl'],
         'path404': book.routes['Tree Slab'],
         'path464': book.routes['Tree Slab'],
         'path480': book.variations['EZe'],
         'path426': book.variations['EZe'],
     })
Topo(name='enchilada',
     parent=book.routes['Enchilada'],
     fileName='enchilada2.svg',
     size='h',
     routes={
         'path2078': book.routes['Enchilada'],
         'path2076': book.routes['Enchilada'],
         'path2086': book.variations['Enchilada Left Project'],
         'path2084': book.variations['Enchilada Left Project'],
         'path2082': book.variations['Enchilada Low Start Project'],
         'path2080': book.variations['Enchilada Low Start Project'],
     })
Topo(name='Slam dunk',
     parent=book.routes['Slam Dunk'],
     fileName='eboulder2.svg',
     size='h',
     routes={
         'path623': book.variations['Layup'],
         'path569': book.variations['Layup'],
         'path1489': book.routes['Slam Dunk'],
         'path1487': book.routes['Slam Dunk'],
     })
Topo(name='Gargoyle',
     parent=book.formations['E\'s Boulder'],
     fileName='eboulder3.svg',
     size='h',
     border='rect292',
     routes={
         'path1625': book.routes['Gargoyle'],
         'path1627': book.routes['Gargoyle'],
         'path1631': book.variations['Gargoyle Direct'],
         'path1629': book.variations['Gargoyle Direct'],
         'path1635': book.routes['Slam Dunk'],
         'path1633': book.routes['Slam Dunk'],
         'path625': book.variations['Layup'],
         'path627': book.variations['Layup'],
     })
Topo(name='Bubonic Plague',
     parent=book.formations['Bubonic Plague'],
     fileName='koan.svg',
     size='h',
     routes={
         'path1777': book.routes['Bubonic Plague'],
         'path1775': book.routes['Bubonic Plague'],
     })
Topo(name='Night Crawler',
     parent=book.formations['Night Crawler'],
     fileName='nightCrawler.svg',
     size='h',
     routes={
         'path1342': book.routes['Night Crawler'],
         'path1396': book.routes['Night Crawler'],
     })
Topo(name='Azain',
     parent=book.routes['Garden Groove'],
     fileName='azainCrack.svg',
     size='h',
     routes={
         'path445': book.routes['Sometimes'],
         'path391': book.routes['Sometimes'],
         'path447': book.routes['Garden Groove'],
         'path293': book.routes['Garden Groove'],
     })
Topo(name='Blowie',
     parent=book.formations['Azain'],
     fileName='blowie.svg',
     size='h',
     routes={
         'path527': book.routes['Ground up Blowie'],
         'path473': book.routes['Ground up Blowie'],
     })
Topo(name='Bubbler',
     parent=book.formations['The Bubbler'],
     fileName='bubbler.svg',
     size='h',
     border='rect1186',
     routes={
         'path1132': book.variations['Chillum Sit'],
         'path1081': book.variations['Chillum Sit'],
         'path1079': book.routes['Chillum'],
         'path1077': book.routes['Chillum'],
         'path1075': book.routes['The Bubbler'],
         'path419': book.routes['The Bubbler'],
     })
Topo(name='Jesus',
     parent=book.formations['Meth Lab'],
     fileName='jesus.svg',
     size='f',
     routes={
         'path1908': book.routes['Don\'t Blow the Jug'],
         'path1910': book.routes['Don\'t Blow the Jug'],
         'path1906': book.routes['Trust Issues'],
         'path1912': book.routes['Trust Issues'],
         'path1902': book.routes['Leave it to Jesus'],
         'path1916': book.routes['Leave it to Jesus'],
         'path1904': book.variations['Leave it to Jesus Sit Start'],
         'path1914': book.variations['Leave it to Jesus Sit Start'],
         'path736': book.variations['Leave it to Jesus Left'],
         'path734': book.variations['Leave it to Jesus Left'],
     })
Topo(name='Methlab Backside',
     parent=book.routes['Smackdown'],
     fileName='Methlab.svg',
     size='f',
     border='rect306',
     routes={
         'path1874': book.routes['Smackdown'],
         'path1856': book.routes['Smackdown'],
         'path1876': book.variations['Harbor Freight'],
         'path1856-2': book.variations['Harbor Freight'],
         'path1878': book.routes['Heisenburg'],
         'path1858': book.routes['Heisenburg'],
         'path1880': book.variations['Learys Lunge'],
         'path1860': book.variations['Learys Lunge'],
         'path1886': book.routes['Octernal'],
         'path1864': book.routes['Octernal'],
         'path1884': book.routes['Guillotine'],
         'path1866': book.routes['Guillotine'],
         'path1888': book.variations['Octernal Center Exit'],
         'path1868': book.variations['Octernal Center Exit'],
         'path1890': book.variations['Octernal Direct Exit'],
         'path1862': book.variations['Octernal Direct Exit'],
         'path1892': book.routes['E\'s'],
         'path1872': book.routes['E\'s'],
         'path1042': book.variations['E\'s Sit Start Project'],
         'path1039': book.variations['E\'s Sit Start Project'],
     })
Topo(name='octernal2',
     parent=book.routes['Two Blows One Stroke'],
     fileName='octurnal2.svg',
     size='h',
     routes={
         'path3604': book.routes['Two Blows One Stroke'],
         'path3553': book.variations['Octernal Direct Exit'],
         'path3608': book.routes['Two Blows One Stroke'],
         'path3606': book.variations['Octernal Direct Exit'],
     })
Topo(name='Locksmith',
     parent=book.formations['Locksmith'],
     fileName='hula.svg',
     size='h',
     routes={
         'path888': book.routes['Philanthropy'],
         'path886': book.routes['Philanthropy'],
         'path883': book.variations['Brain Haemorrhage'],
         'path879': book.variations['Brain Haemorrhage'],
         'path881': book.routes['Locksmith'],
         'path877': book.routes['Locksmith'],
     })
Topo(name='Philanthropy',
     parent=book.routes['Philanthropy'],
     fileName='philanthropy.svg',
     size='h',
     routes={
         'path1321': book.variations['Brain Haemorrhage'],
         'path1319': book.variations['Brain Haemorrhage'],
         'path1317': book.routes['Philanthropy'],
         'path1315': book.routes['Philanthropy'],
         'path1313': book.routes['Philanthropy']
     })
# Topo(name='arboretum',
#      parent=book.routes['Garden Project'],
#      fileName='arboretum.svg',
#      size='h',
#      routes={
#          'path501': book.routes['Garden Variety'],
#          'path505': book.routes['Garden Project'],
#          'path293': book.routes['The Arboretum'],
#          'path349': book.routes['The Other Berned'],
#          'path512': book.routes['Garden Variety'],
#          'path510': book.routes['Garden Project'],
#          'path508': book.routes['The Arboretum'],
#          'path502': book.routes['The Arboretum'],
#          'path508-1': book.routes['The Other Berned'],
#          'path350': book.variations['Arboretum Sit Start'],
#          'path404': book.variations['Arboretum Sit Start'],
#      })
Topo(name='arboretum2',
     parent=book.routes['The Arboretum'],
     fileName='childOfGod.svg',
     size='f',
     routes={
         'path1492': book.routes['Gumby Arête'],
         'path1490': book.routes['Gumby Arête'],
         'path1488': book.variations['Bag of Tricks'],
         'path1486': book.variations['Bag of Tricks'],
         'path1484': book.routes['The Siren'],
         'path1482': book.routes['The Siren'],
         'path1480': book.routes['Somewhere In-Between'],
         'path1478': book.routes['Somewhere In-Between'],
         'path1476': book.routes['Somewhere In-Between'],
         'path1474': book.routes['The Other Berned'],
         'path1472': book.routes['The Other Berned'],
         'path1470': book.routes['The Arboretum'],
         'path1468': book.routes['The Arboretum'],
         'path1466': book.routes['The Arboretum'],
         'path1464': book.routes['Garden Project'],
         'path1462': book.routes['Garden Project'],
         'path1460': book.routes['Garden Variety'],
         'path903': book.routes['Garden Variety'],
     })
# Topo(name='siren',
#      parent=book.formations['Gumby Wall'],
#      fileName='siren.svg',
#      size='h',
#      routes={
#          'path5162': book.variations['The Siren Stand Start'],
#          'path4111': book.routes['The Siren'],
#          'path4117': book.routes['Gumby Arête'],
#          'path4212': book.routes['The Other Berned'],
#          'path4115': book.variations['Bag of Tricks'],
#          'path4175': book.routes['The Siren'],
#          'path4175-8': book.routes['Gumby Arête'],
#          'path4175-8-6': book.routes['The Other Berned'],
#          'path4175-2': book.variations['Bag of Tricks'],
#          'path4175-8-6-6': book.variations['The Siren Stand Start'],
#      })
Topo(name='Chockstone',
     parent=book.formations['Chockstone Highball'],
     fileName='chockstone.svg',
     size='h',
     routes={
         'path1669': book.routes['Chockstone Highball'],
         'path1667': book.routes['Chockstone Highball'],
     })
Topo(name='All Sorts of Ease',
     parent=book.formations['All Sorts of Ease'],
     fileName='ease.svg',
     size='h',
     routes={
         'path1938': book.routes['In the Shadow of Giants'],
         'path1936': book.routes['In the Shadow of Giants'],
         'path1934': book.routes['All Sorts of Ease'],
         'path1932': book.routes['All Sorts of Ease'],
     })
Topo(name='scaryGrandma',
     parent=book.routes['Scary Grandma'],
     fileName='scaryGrandma.svg',
     size='h',
     routes={
         'path609': book.routes['Scary Grandma'],
         'path603': book.routes['Scary Grandma'],
         'path611': book.routes['Angry Mom'],
         'path605': book.routes['Angry Mom'],
         'path613': book.routes['Easy Grandma'],
         'path607': book.routes['Easy Grandma'],
     })
Topo(name='scaryGrandma2',
     parent=book.formations['Scary Grandma'],
     fileName='angryMom.svg',
     size='h',
     routes={
         'path447': book.routes['Easy Grandma'],
         'path391': book.routes['Easy Grandma'],
         'path445': book.routes['Angry Mom'],
         'path293': book.routes['Angry Mom'],
     })
#Topo(name='Gumby',
#     parent=book.routes['Gumby Arête'],
#     fileName='gumby.svg',
#     size='h',
#     routes={
#         'path1571': book.variations['Bag of Tricks'],
#         'path1573': book.variations['Bag of Tricks'],
#         'path1678': book.variations['Bag of Tricks'],
#         'path1569': book.routes['Gumby Arête'],
#         'path1676': book.routes['Gumby Arête'],
#         'path1563': book.routes['Gumby Slab'],
#         'path1674': book.routes['Gumby Slab'],
#     })
Topo(name='Mini Me',
     parent=book.formations['Mini Me'],
     fileName='miniMe.svg',
     size='h',
     border='rect505',
     routes={
         'path451': book.routes['Austin Powers'],
         'path449': book.routes['Austin Powers'],
         'path447': book.routes['Austin Powers'],
         'path445': book.routes['Mini Me'],
         'path391': book.routes['Mini Me'],
         'path293': book.routes['Mini Me'],
     })
Topo(name='Mini Me 3',
     parent=book.routes['Dr. Evil'],
     fileName='miniMe3.svg',
     size='h',
     routes={
         'path1844': book.routes['Dr. Evil'],
         'path1842': book.routes['Dr. Evil'],
         'path1850': book.routes['Dr. Evil'],
         'path1846': book.routes['Austin Powers'],
         'path1852': book.routes['Austin Powers'],
         'path1848': book.routes['Mini Me'],
         'path1854': book.routes['Mini Me'],
     })
Topo(name='Office',
     parent=book.formations['Crash Test Dummies'],
     fileName='office.svg',
     size='f',
     border='rect568',
     routes={
         'path460': book.routes['Dwight Schrute'],
         'path458': book.routes['Dwight Schrute'],
         'path456': book.routes['Jim Halpert'],
         'path454': book.routes['Jim Halpert'],
         'path452': book.routes['Michael Scott'],
         'path351': book.routes['Michael Scott'],
         'path449': book.routes['Daryl Philbin'],
         'path349': book.routes['Daryl Philbin'],
         'path347': book.routes['Vince'],
         'path293': book.routes['Vince'],
     })
Topo(name='Swollen Member',
     parent=book.formations['Swollen Member'],
     fileName='swollen2.svg',
     size='h',
     routes={
         'path1813': book.routes['Meth Lab Highball'],
         'path2042': book.routes['Meth Lab Highball'],
         'path1830': book.variations['Meth Lab Highball Sit Start'],
         'path2040': book.variations['Meth Lab Highball Sit Start'],
         'path1832': book.routes['Meth Lab Highball Right'],
         'path2044': book.routes['Meth Lab Highball Right'],
         'path1760': book.routes['Swollen Member'],
         'path1811': book.routes['Swollen Member'],
         'path2046': book.routes['Swollen Member'],
     })
Topo(name='Toilet Bowl',
     parent=book.formations['Toilet Bowl'],
     fileName='toilet.svg',
     size='h',
     routes={
         'path1036': book.routes['Toilet Bowl Traverse'],
         'path1034': book.routes['Toilet Bowl Traverse'],
         'path1026': book.routes['Toilet Bowl'],
         'path1024': book.routes['Toilet Bowl'],
     })
Topo(name='Tonsil',
     parent=book.routes['Tonsil'],
     fileName='tonsil2.svg',
     size='h',
     routes={
         'path2317': book.routes['Gingiva'],
         'path2315': book.routes['Gingiva'],
         'path2313': book.variations['Prowed'],
         'path2311': book.variations['Prowed'],
         'path2309': book.routes['Tonsil'],
         'path2307': book.routes['Tonsil'],
         'path2305': book.routes['Tonsil'],
     })
Topo(name='Geodesic Weiner',
     parent=book.formations['François'],
     fileName='weiner.svg',
     size='h',
     routes={
         'path2460': book.routes['Geodesic Weiner'],
         'path2458': book.routes['Geodesic Weiner'],
     })
Topo(name='Trust',
     parent=book.formations['Tyler Durten'],
     fileName='trust.svg',
     size='h',
     routes={
         'path678': book.routes['Trust'],
         'path830': book.routes['Trust'],
         'path776': book.variations['Iron Cross'],
         'path832': book.variations['Iron Cross'],
         'path834': book.routes['Project Mayhem'],
         'path836': book.routes['Project Mayhem'],
         'path838': book.variations['Tyler Durten Dyno'],
         'path840': book.variations['Tyler Durten Dyno'],
         'path298': book.routes['Angel Face'],
         'path410': book.routes['Angel Face'],
         'path302': book.routes['Durten Layback'],
         'path412': book.routes['Durten Layback'],
     })
Topo(name='Turtle',
     parent=book.formations['Turtle Shell Boulder'],
     fileName='turtle.svg',
     size='h',
     routes={
         'path855': book.routes['Raphael Crack'],
         'path861': book.routes['Raphael Crack'],
         'path857': book.routes['Donatello'],
         'path863': book.routes['Donatello'],
         'path859': book.routes['Leonardo'],
         'path919': book.routes['Leonardo'],
     })
Topo(name='Undertow',
     parent=book.routes['Undertow'],
     fileName='undertow.svg',
     size='h',
     routes={
         'path1199': book.routes['Spray Skirt'],
         'path1201': book.routes['Spray Skirt'],
         'path1097': book.routes['Undertow'],
         'path1111': book.routes['Undertow'],
         'path1099': book.variations['Spray Against the Undertow'],
         'path1105': book.variations['Spray Against the Undertow'],
         'path1101': book.variations['Undertow Sit Start'],
         'path1107': book.variations['Undertow Sit Start'],
         'path1103': book.routes['Riptide'],
         'path1109': book.routes['Riptide'],
     })
Topo(name='Undertow2',
     parent=book.formations['Silly Steep'],
     fileName='undertow2.svg',
     size='f',
     routes={
         'path932': book.routes['Silly Steep Mantle'],
         'path942': book.routes['Silly Steep Mantle'],
         'path938': book.routes['Undertow'],
         'path946': book.routes['Undertow'],
         'path934': book.variations['Spray Against the Undertow'],
         'path944': book.variations['Spray Against the Undertow'],
         'path936': book.variations['Undertow Sit Start'],
         'path950': book.variations['Undertow Sit Start'],
         'path1134': book.routes['Riptide'],
         'path948': book.routes['Riptide'],
         'path940': book.routes['Tidepool'],
         'path952': book.routes['Tidepool'],
         'path1309': book.routes['Simple Math'],
         'path1305': book.routes['Simple Math'],
         'path1307': book.variations['Shake it Out'],
         'path748': book.variations['Shake it Out'],
     })
Topo(name='François',
     parent=book.routes['François'],
     fileName='frank.svg',
     size='h',
     loc='e',
     routes={
         'path2390': book.routes['François'],
         'path2388': book.routes['François'],
     })
Topo(name='Oregon Arête',
     parent=book.formations['Garden Roof'],
     fileName='oregonArete.svg',
     size='h',
     routes={
         'path742': book.routes['Full Stroke'],
         'path737': book.routes['Full Stroke'],
         'path739': book.variations['Dream Weaver'],
         'path735': book.variations['Dream Weaver'],
         'path733': book.routes['Oregon Arête'],
         'path731': book.routes['Oregon Arête'],
         'path729': book.routes['Oregon Arête'],
     })
Topo(name='Full Stroke',
     parent=book.routes['Full Stroke'],
     fileName='fullStroke.svg',
     size='h',
     border='rect444',
     routes={
         'path1215': book.variations['Sebulba'],
         'path1207': book.variations['Sebulba'],
         'path1212': book.routes['Eurovision'],
         'path1205': book.routes['Eurovision'],
         'path1217': book.variations['Dream Weaver'],
         'path1203': book.variations['Dream Weaver'],
         'path1209': book.routes['Full Stroke'],
         'path1201': book.routes['Full Stroke'],
     })
Topo(name='The Good Warmup',
     parent=book.formations['The Good Warmup'],
     fileName='goodWarmUp.svg',
     size='h',
     routes={
         'path579': book.routes['The Good Warmup'],
         'path581': book.routes['The Good Warmup'],
     })
Topo(name='Gumby Wall',
     parent=book.routes['Gumby Arête'],
     fileName='gumby.svg',
     size='f',
     routes={
         'path643': book.variations['Bag of Tricks'],
         'path594': book.variations['Bag of Tricks'],
         'path434': book.variations['Bag of Tricks'],
         'path432': book.routes['Gumby Arête'],
         'path430': book.routes['Gumby Arête'],
         'path428': book.routes['Gumby Slab'],
         'path426': book.routes['Gumby Slab'],
         'path424': book.routes['Gumby Crack'],
         'path370': book.routes['Gumby Crack'],
     })
Topo(name='Hanging Prow Project',
     parent=book.formations['Hanging Prow'],
     fileName='prowProj.svg',
     size='h',
     routes={
         'path1600': book.routes['Hanging Prow Project'],
         'path1598': book.routes['Hanging Prow Project'],
     })
Topo(name='Small',
     parent=book.formations['Small'],
     fileName='small.svg',
     size='h',
     routes={
         'path1740': book.routes['Smol'],
         'path1738': book.routes['Smol'],
         'path1736': book.routes['Smol'],
     })
Topo(name='Spider Bumps',
     parent=book.routes['Spider Bumps'],
     fileName='spiderBumps.svg',
     size='h',
     routes={
         'path1879': book.routes['Spider Bumps'],
         'path1877': book.routes['Spider Bumps'],
     })
Topo(name='Farley',
     parent=book.routes['Belushi'],
     fileName='farley.svg',
     size='f',
     routes={
         'path2245': book.routes['Farley Prep'],
         'path2243': book.routes['Farley Prep'],
         'path2241': book.routes['Farley Prep'],
         'path2239': book.routes['Le Lemét'],
         'path2237': book.routes['Le Lemét'],
         'path2232': book.routes['Lippity Split'],
         'path2228': book.routes['Lippity Split'],
         'path2230': book.routes['Belushi'],
         'path2226': book.routes['Belushi'],
     })
Topo(name='Frat House',
     parent=book.formations['Frat House'],
     fileName='fratHouse.svg',
     size='h',
     border='rect1761',
     routes={
         'path1063': book.routes['Frat Mouse'],
         'path1061': book.routes['Frat Mouse'],
         'path1059': book.routes['Frat House'],
         'path1057': book.routes['Frat House'],
     })
Topo(name='Jonah\'s Dab Rig',
     parent=book.formations['Jonah\'s Dab Rig'],
     fileName='jonah.svg',
     size='h',
     routes={
         'path1028': book.routes['Jonah\'s Dab Rig'],
         'path1030': book.routes['Jonah\'s Dab Rig'],
         'path1032': book.routes['Jonah\'s Dab Rig'],
         'path1034': book.variations['Workshop 68'],
         'path1036': book.variations['Workshop 68'],
     })
Topo(name='Territorial Pissings',
     parent=book.formations['Pissing Boulder'],
     fileName='pissings.svg',
     size='h',
     routes={
         'path1462': book.routes['Territorial Pissings'],
         'path1460': book.routes['Territorial Pissings'],
     })
Topo(name='Pumpkin',
     parent=book.formations['Pumpkin'],
     fileName='pSpice.svg',
     size='h',
     routes={
         'path887': book.routes['Pumpkin Spice'],
         'path885': book.routes['Pumpkin Spice'],
         'path883': book.routes['Pumpkin Spice'],
     })
Topo(name='Baseball',
     parent=book.formations['Baseball'],
     fileName='baseball.svg',
     size='h',
     routes={
         'path349': book.routes['Baseball'],
         'path293': book.routes['Baseball'],
         'path351': book.routes['Bunt'],
         'path295': book.routes['Bunt'],
     })
Topo(name='Bread Loaf',
     parent=book.routes['Worf'],
     fileName='breadLoaf.svg',
     size='f',
     border='rect1397',
     routes={
         'path543': book.routes['Bread Loaf Left'],
         'path1100': book.routes['Bread Loaf Left'],
         'path1104': book.routes['Bread Loaf Traverse'],
         'path1153': book.routes['Bread Loaf Traverse'],
         'path390': book.routes['Breadwinner'],
         'path444': book.routes['Breadwinner'],
         'path1155': book.variations['Baker\'s Dozen'],
         'path1157': book.variations['Baker\'s Dozen'],
     })
Topo(name='Bread Loaf2',
     parent=book.routes['Worf'],
     fileName='breadLoaf2.svg',
     size='h',
     loc='e',
     routes={
         'path1350': book.routes['Worf'],
         'path1348': book.routes['Worf'],
         'path1346': book.routes['Worf'],
     })
Topo(name='Scratch and Spliff',
     parent=book.routes['Spliff'],
     fileName='scratchSpliff.svg',
     size='h',
     routes={
         'path1921': book.routes['Spliff'],
         'path1919': book.routes['Spliff'],
         'path1925': book.routes['Scratch and Spliff Traverse'],
         'path1923': book.routes['Scratch and Spliff Traverse'],
         'path1917': book.routes['Roach'],
         'path1915': book.routes['Roach'],
         'path2515': book.routes['For What it\'s Worth'],
         'path2513': book.routes['For What it\'s Worth'],
     })
Topo(name='Scratch and Spliff 2',
     parent=book.formations['Scratch and Spliff'],
     fileName='scratchSpliff2.svg',
     size='f',
     routes={
         'path2067': book.routes['Scratch and Spliff Traverse'],
         'path2065': book.routes['Scratch and Spliff Traverse'],
         'path2080': book.routes['Spliff'],
         'path2075': book.routes['Spliff'],
         'path2077': book.routes['Scratch'],
         'path2073': book.routes['Scratch'],
         'path2071': book.variations['Late Start'],
         'path2069': book.variations['Late Start'],
     })
Topo(name='Scratch and Spliff 3',
     parent=book.routes['Caliban\'s War'],
     fileName='scratchSpliff3.svg',
     size='h',
     routes={
         'path2226': book.routes['Caliban\'s War'],
         'path2224': book.routes['Caliban\'s War'],
         'path2222': book.routes['Caliban\'s War'],
         'path2228': book.routes['Stoned Age'],
         'path2230': book.routes['Stoned Age'],
     })
Topo(name='4.5',
     parent=book.formations['The 4.5'],
     fileName='45.svg',
     size='f',
     border='rect1383',
     routes={
         'path1329': book.routes['Falcon\'s Reach'],
         'path1323': book.routes['Falcon\'s Reach'],
         'path1327': book.routes['Teenage Libertarians'],
         'path1321': book.routes['Teenage Libertarians'],
         'path1325': book.routes['Chicken Tendies'],
         'path764':  book.routes['Chicken Tendies'],
     })
AreaMap(name='The Garden Main Area Overview',
        parent=book.areas['The Garden Main'],
        fileName='Garden.svg',
        sub_areas={
            'rect2027': book.subareas['Big'],
            'rect1376': book.subareas['Azain'],
            'rect2313': book.subareas['Entrance Area'],
            'rect2908': book.subareas['François'],
            'rect3027': book.subareas['Fight Club'],
            'rect3200': book.subareas['Meth Lab'],
            'rect3386': book.subareas['Undertow'],
            'rect380': book.subareas['Child of God'],
        },
        layers=['Base'],
        size='f',)
AreaMap(name='Armageddon Area Overview',
        parent=book.areas['Upper Garden'],
        fileName='upperGarden.svg',
        sub_areas={
            'rect27128': book.subareas['The Bread Loaves/Scratch and Spliff'],
            'rect27051': book.subareas['entranceUpper'],
        },
        layers=['Base', 'border'],
        border='rect27048',
        size='f',)
SubAreaMap(name='Entrance Area map',
           parent=book.subareas['Entrance Area'],
           outFileName='entrance_c.png',
           fileName='Garden.svg',
           layers=['Entrance', 'Base'],
           border='rect2313',
           routes={
           'path1767': book.routes['Toilet Bowl Traverse'],
           'path1769': book.routes['Toilet Bowl'],
           'path1771': book.routes['Donatello'],
           'path1773': book.routes['Leonardo'],
           'path1775': book.routes['Raphael Crack'],
           'path1777': book.routes['Overhand'],
           'path1779': book.routes['Three Star Ledge'],
           'path1781': book.routes['All Sorts of Ease'],
           'path1783': book.routes['In the Shadow of Giants'],
           'path1785': book.routes['Tonsil'],
           'path1787': book.routes['Boys in the Woods'],
           'path1789': book.routes['Ice Cubes Shiny Jerry Curl'],
           'path1791': book.routes['Tree Slab'],
           'path2200': book.routes['Spider Bumps'],
           })
SubAreaMap(name='Fight Club Area map',
           parent=book.subareas['Fight Club'],
           outFileName='fightClub_c.png',
           fileName='Garden.svg',
           layers=['Fight Club', 'Base'],
           border='rect3027',
           size='f',
           routes={
           'path876': book.routes['Unknown on E\'s Dirty B'],
           'path874': book.routes['Green Hell'],
           'path4077': book.routes['E\'s Dirty B'],
           'path1519': book.routes['Jim Halpert'],
           'path878': book.routes['Michael Scott'],
           'path1517': book.routes['Daryl Philbin'],
           'path1515': book.routes['Vince'],
           'path1513': book.routes['The Ear'],
           'path1511': book.routes['Fight Club 2'],
           'path1509': book.routes['Project Mayhem'],
           'path1507': book.routes['Trust'],
           'path1505': book.routes['Austin Powers'],
           'path1503': book.routes['Dr. Evil'],
           'path1174': book.routes['Dwight Schrute'],
           })
SubAreaMap(name='Undertow area map',
           parent=book.subareas['Undertow'],
           outFileName='undertow_c.png',
           fileName='Garden.svg',
           layers=['Undertow', 'Base'],
           border='rect3386',
           routes={
               'path1244': book.routes['Silly Steep Mantle'],
               'path1246': book.routes['Undertow'],
               'path1248': book.routes['Tidepool'],
               'path1250': book.routes['Chockstone Highball'],
               'path1252': book.routes['Car Alarm Traverse'],
               'path1254': book.routes['2 Ton Chevey'],
               'path1256': book.routes['Pup Truck'],
               'path1258': book.routes['Comp Route'],
               'path1260': book.routes['Panic Button'],
               'path3384': book.routes['Bubonic Plague'],
               'path872': book.routes['Hanging Prow Project'],
           })
SubAreaMap(name='Meth Lab area map',
           parent=book.subareas['Meth Lab'],
           outFileName='methLab_c.png',
           fileName='Garden.svg',
           layers=['Meth Lab', 'Base'],
           border='rect3200',
           routes={
               'path760': book.routes['Don\'t Blow the Jug'],
               'path762': book.routes['Trust Issues'],
               'path764': book.routes['Leave it to Jesus'],
               'path766': book.routes['Smackdown'],
               'path768': book.routes['Heisenburg'],
               'path770': book.routes['Guillotine'],
               'path772': book.routes['Octernal'],
               'path774': book.routes['Two Blows One Stroke'],
               'path776': book.routes['E\'s'],
               'path778': book.routes['Enchilada'],
               'path780': book.routes['Swollen Member'],
               'path782': book.routes['Meth Lab Highball'],
               'path784': book.routes['The Bubbler'],
               'path4341': book.routes['Meth Lab Project'],
               'path426': book.routes['Slam Dunk'],
               'path428': book.routes['Gargoyle'],
               'path1731': book.routes['Chillum'],
           })
SubAreaMap(name='Big area map',
           parent=book.subareas['Big'],
           outFileName='big_c.png',
           fileName='Garden.svg',
           routes={
               'path5664': book.routes['Frontside Baldo'],
               'path5662': book.routes['Hueco Wabo'],
               'path5660': book.routes['All Berned Up'],
               'path410': book.routes['Mini Hydro Tube'],
               'path5658': book.routes['Bitchin Corners'],
               'path5658-4': book.routes['Smol'],
               'path880': book.routes['Cargo Net Project'],
               'path1395': book.routes['Classique'],
               'path1397': book.routes['Crazy Cool Arête'],
           },
           layers=['Big', 'Base'],
           border='rect2027'),
SubAreaMap(name='Azain area map',
           parent=book.subareas['Azain'],
           outFileName='azain_c.png',
           fileName='Garden.svg',
           size='h',
           routes={
               'path720': book.routes['Ground up Blowie'],
               'path718': book.routes['Night Crawler'],
               'path716': book.routes['The Good Slab'],
               'path714': book.routes['The Good'],
               'path712': book.routes['Another'],
               'path710': book.routes['Snakes and Martyrs'],
               'path708': book.routes['Next to the Good'],
               'path740': book.routes['Into the Light'],
               'path742': book.routes['Garden Groove'],
               'path1962': book.routes['Simpson\'s First'],
           },
           layers=['Azain', 'Base'],
           border='rect1376')
SubAreaMap(name='Child of God area map',
           parent=book.subareas['Child of God'],
           outFileName='child_o_god_c.png',
           fileName='Garden.svg',
           size='f',
           routes={
               'ellipse374': book.routes['Gumby Crack'],
               'circle372': book.routes['Gumby Slab'],
               'circle370': book.routes['Gumby Arête'],
               'ellipse368': book.routes['The Siren'],
               'ellipse366': book.routes['Garden Variety'],
               'ellipse364': book.routes['The Arboretum'],
               'ellipse362': book.routes['Full Stroke'],
               'ellipse360': book.routes['Philanthropy'],
               'circle358': book.routes['Locksmith'],
               'path1391': book.routes['Oregon Arête'],
           },
           layers=['Child of God', 'Base'],
           border='rect380')
SubAreaMap(name='François area map',
           parent=book.subareas['François'],
           outFileName='Big Frank_c.png',
           fileName='Garden.svg',
           layers=['Big Fred', 'Base'],
           border='rect2908',
           routes={
               'path714-5': book.routes['Scary Grandma'],
               'path712-7': book.routes['Angry Mom'],
               'path712-6': book.routes['Easy Grandma'],
               'path710-5': book.routes['François'],
               'path1964': book.routes['Geodesic Weiner'],
               'path1966': book.routes['Shake it Out Frank'],
           })
SubAreaMap(name='Entrance area map',
           parent=book.subareas['entranceUpper'],
           outFileName='entranceUpper_c.png',
           fileName='upperGarden.svg',
           layers=['Entrance', 'Base'],
           border='rect27051',
           routes={
               'path27105': book.routes['Pumpkin Spice'],
               'path27107': book.routes['Baseball'],
               'path27109': book.routes['Bunt'],
           })
SubAreaMap(name='Bread loaf/Scratch and Spliff area map',
           parent=book.subareas['The Bread Loaves/Scratch and Spliff'],
           outFileName='bread_c.png',
           fileName='upperGarden.svg',
           layers=['Bread', 'Base'],
           border='rect27128',
           size='f',
           routes={
               'path27126': book.routes['Caliban\'s War'],
               'path27124': book.routes['Scratch'],
               'path27122': book.routes['Spliff'],
               'path27120': book.routes['Scratch and Spliff Traverse'],
               'path27118': book.routes['For What it\'s Worth'],
               'path27116': book.routes['Worf'],
               'path27114': book.routes['Bread Loaf Traverse'],
               'path27112': book.routes['Bread Loaf Left'],
           })
SubAreaMap(name='Redneck Riviera area map',
           parent=book.subareas['Redneck Riviera'],
           fileName='redneck.svg',
           layers=['Redneck', 'Base'],
           border='rect3479',
           size='h',
           routes={
               'path3423': book.routes['Pony Boy'],
               'path3421': book.routes['Monorail Project'],
               'path3419': book.routes['Ugly Face'],
               'path3417': book.routes['Binding of Isaac'],
               'path3415': book.routes['Moss Boss'],
               'path3413': book.routes['Chicken Tendies'],
               'path3411': book.routes['Teenage Libertarians'],
               'path3357': book.routes['Falcon\'s Reach'],
           })
           
           
if __name__ == '__main__':
    sys.exit()